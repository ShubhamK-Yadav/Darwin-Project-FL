...
def load_pretrained_weights(model, pretrained_backbone='resnet34'):
    """
    Initializes the encoder part of the UNet or ITUNet model with pretrained weights.
    
    Args:
        model: The UNet or ITUNet model to initialize
        pretrained_backbone: The name of the pretrained backbone to use. Options: 'resnet18', 'resnet34', 'resnet50'
    ...
    try:
        ...
        elif pretrained_backbone == 'resnet34':
            pretrained = models.resnet34(weights='IMAGENET1K_V1')
        ...
        print(f"Loading pretrained weights from {pretrained_backbone}")
        
        # Instead of trying to copy the weights directly (which causes dimension mismatches),
        # we'll perform proper initialization of all layers
        
        # Print model structure to help debugging
        print(f"Model input channels: {model.n_channels}")
        
        # Apply careful He initialization to all layers
        for m in model.modules():
            if isinstance(m, nn.Conv2d):
                # Use kaiming initialization with a small factor to reduce extreme values
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                # Scale down weights slightly to prevent initial instability
        ...
        print("Applied conservative He initialization to all model layers")
        return model
        
    except Exception as e:
        print(f"Error during loading pretrained weights: {e}")
        print("Falling back to conservative initialization for all layers")
        
        # Apply conservative initialization to all layers
        for m in model.modules():
            if isinstance(m, nn.Conv2d):
                # Use a more conservative initialization
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                # Scale down weights to prevent large initial values
                m.weight.data = m.weight.data * 0.1
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
            elif isinstance(m, nn.BatchNorm2d):
...
def initialize_weights(model):
    """
    Apply improved weight initialization to model layers.
    This is especially useful for layers that don't get pretrained weights.
    
    Args:
        model: The model to initialize
    
    Returns:
        model: The model with initialized weights
    """
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
...
def itunet_2d(**kwargs):
    model = ITUNet_2d(stem=DoubleConv2D,
                down=Down2D,
                up=Up2D,
                tail=Tail2D,
